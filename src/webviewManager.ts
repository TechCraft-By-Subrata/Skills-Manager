...
export interface SourceMessage {
  command: 'add' | 'delete' | 'sync' | 'refresh';
  index?: number;
  data?: {
    repo: string;
    path: string;
...
export class WebviewManager {
  ...
  private onMessageHandler: ((msg: SourceMessage) => void) | undefined;
...
<body>
  ...
  createPanel(context: vscode.ExtensionContext, onMessage: (msg: SourceMessage) => void): void {
  ...
  registerDashboardView(context: vscode.ExtensionContext, viewId: string): void {
    context.subscriptions.push(
      vscode.window.registerWebviewViewProvider(viewId, {
        resolveWebviewView: (webviewView: vscode.WebviewView) => {
          ...
          webviewView.webview.onDidReceiveMessage((msg: SourceMessage) => {
  ...
  setOnMessageHandler(onMessage: (msg: SourceMessage) => void): void {
    this.onMessageHandler = onMessage;
  }

  updateSources(sources: SourceData[]): void {
    if (this.panel) {
